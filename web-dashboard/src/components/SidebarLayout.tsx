...
// Only re-render if pathname, adminProfileImage, adminName, or onLogout changes
const SidebarContent = memo(function SidebarContent({ 
  ...
  return (
    <div className="w-64 bg-white shadow-lg relative flex flex-col h-screen sidebar-persistent">
      <div className="relative z-10 flex flex-col h-full">
        {/* Header - Fixed */}
        <div className="p-6 flex-shrink-0">
          <h1 className="text-2xl font-bold text-purple-600">Sufar</h1>
          <div className="mt-1">
          ...
          <div className="space-y-2 pb-4">
            {navigationItems.map((item) => {
              ...
              return (
                ...
                >
                  <span className={`mr-3 transition-colors ${isActive ? 'text-white' : 'text-gray-500 group-hover:text-purple-600'}`}>
                    {item.icon}
                  </span>
                  {item.name}
                </Link>
        ...
        <div className="flex-shrink-0 p-6 border-t border-gray-200 bg-white relative z-10">
          <div className="flex items-center">
            <div className="flex-shrink-0">
              <div className="w-12 h-12 bg-purple-600 rounded-full flex items-center justify-center overflow-hidden shadow-md ring-2 ring-white/50">
                {adminProfileImage ? (
                  <img
            ...
            <div className="ml-4 flex-1">
              ...
              <button
                onClick={onLogout}
                className="mt-2 px-3 py-1.5 text-sm font-medium text-white bg-purple-600 hover:bg-purple-700 rounded-md transition-all duration-200 shadow-sm hover:shadow-md transform hover:scale-105"
              >
                Sign out
              </button>
            </div>
...
// Main SidebarLayout component - memoized to prevent re-renders
// Only re-render if adminName changes, not when children change
const SidebarLayoutComponent = memo(function SidebarLayoutComponent({ children, adminName }: SidebarLayoutProps) {
  ...
  // Memoize the handleLogout function to prevent re-creation
  ...
  return (
    <div className="h-screen bg-gray-50 flex overflow-hidden">
      {/* Sidebar - Fixed height, no scrolling - Memoized to prevent re-renders */}
      <SidebarContent
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <main className="flex-1 overflow-y-auto main-content page-transition">
          {children}
        </main>
      </div>
  ...
  // Only re-render if adminName changes, not when children change
  // This ensures sidebar stays persistent while content changes
  return prevProps.adminName === nextProps.adminName;
});

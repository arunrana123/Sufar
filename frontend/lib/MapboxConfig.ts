// MAPBOX CONFIGURATION - Access token and map settings for user app
// Features: Map initialization, directions API, location tracking, place search
...
// Lazy initialize Mapbox - only called when map screen loads
...
export const initializeMapbox = () => {
  try {
    ...
    if (!tokenFromEnv) {
      console.warn('⚠️ Missing EXPO_PUBLIC_MAPBOX_ACCESS_TOKEN. Add it to frontend/.env');
      return false;
    }
    ...
    console.log('✅ Mapbox initialized for user app');
    return true;
  } catch (error) {
...
// Triggered by: User viewing route to service provider
